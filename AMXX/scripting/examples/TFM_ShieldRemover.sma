...
public plugin_init()
{
	...
	register_touch("weapon_shield", "player", "fw_TouchWeaponShield")
	
	RegisterHam(Ham_Spawn, "weapon_shield", "HamFW_ShieldSpawn")
}

public HamFW_ShieldSpawn(iEnt)
{
	if (!pev_valid(iEnt))
		return HAM_IGNORED
...
public fw_SetModelPre(iEnt, szModel[])
{
	...
	if (!equal(szModel, "models/w_shield.mdl"))
		return FMRES_IGNORED
	
	return FMRES_SUPERCEDE
}

public fw_TouchWeaponShield(iEnt, id)
{
	if (!pev_valid(iEnt))
		return PLUGIN_CONTINUE
