...
//	STORE INFORMATION OF GRENADE : SOUND, HUD, MODEL, ....
...
public _give_user_grenade(id, iGrenadeId)
{
	...
	if (iGrenadeId > g_weapon_count - 1 || iGrenadeId < 0)
		return 0
	
	new iVip = cs_get_user_vip(id)
	
	new iEquipMethod 
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_EQUIP_METHOD, iEquipMethod)
...
public _find_grenade_by_serial(szSerial[])
{
	...
	{
		new szRealSerial[64]
		TrieGetString(weapon_StringInfo[i], SECTION_WPN_SERIAL, szRealSerial, sizeof szRealSerial - 1)
...
public _get_grenade_level(iGrenadeId)
{
	...
	new iGrenadeLevel
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_LEVEL, iGrenadeLevel)
...
public _get_grenade_cost(iGrenadeId)
{
	...
	new iCost
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_COST, iCost)
	return iCost
}
...
public _get_grenade_cost_type(iGrenadeId)
{
	...
	new iCostType
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_COST_TYPE, iCostType)
	return iCostType
}
...
public _get_grenade_hud(iGrenadeId, szHud[], iLen)
{
	...
	TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_WPN_HUD, szHud, iLen)
}
...
public Float:_get_grenade_weight(iGrenadeId)
{
	...
	new Float:fWeight
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_WPN_WEIGHT, fWeight)
	return fWeight
}
...
public Float:_get_grenade_speed(iGrenadeId)
{
	...
	new Float:fSpeed
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_SPEED_REDUCTION ,fSpeed)
	return fSpeed
}
...
public Float:_get_grenade_kb_power(iGrenadeId)
{
	...
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_KNOCKBACK_POWER, fKnockBackPower)
...
public _get_grenade_name(iGrenadeId, szName[], iLen)
{
	...
	TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_WEAPON_NAME, szName, iLen)
}
...
public plugin_init() 
{
	...
	for (new iGrenadeId = 0; iGrenadeId < MAX_WPN; iGrenadeId++)
		weapon_StringInfo[iGrenadeId] = TrieCreate()
...
public csred_WpnAttachToPlayerPost(id, iEnt, iWeaponId)
{
	...
	TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_WPN_CLASS, szWeaponClass, sizeof szWeaponClass - 1)
	UT_UpdateWpnList(id, iWeaponId, 1, szWeaponClass, 1, 0)
}
...
public csred_WpnDeploy_Post(id, iEnt, iWeaponId)
{
	...
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_DEPLOY_TIME, fDeployTime)
	
	if (fDeployTime > 0.0)
	{
		fm_set_weapon_next_attack(iEnt,fDeployTime)
	...
	TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_V_MODEL, szModelBuffer, sizeof szModelBuffer - 1)
	set_pev(id, pev_viewmodel2, szModelBuffer)
	
	
	TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_P_MODEL, szModelBuffer, sizeof szModelBuffer - 1)
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_P_SUB, iSubBody)
...
public csred_WpnHolster_Post(id, iEnt)
{
	...
	new iSubBody
	TrieGetCell(weapon_StringInfo[iGrenadeId], SECTION_BACK_MODEL, iSubBody)
	
	if (iSubBody > -1)
	{
		new szBackModel[128]
		TrieGetString(weapon_StringInfo[iGrenadeId], SECTION_BACK_MODEL, szBackModel, sizeof szBackModel - 1)
...
public fw_SetModelPost(iEnt, szModel[])
{
	...
	else if (equal(szClassName, "weaponbox"))
	{
		...
		else
		{
			...
			if (iGrenadeId > g_weapon_count - 1)
					return
