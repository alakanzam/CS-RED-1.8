...
stock is_ads_weapon(iWeaponId)
	return UT_Get_CS_ADS_State(iWeaponId)
...
public plugin_natives()
{
	...
	register_native("is_valid_sw", "native_is_valid_sw", 1)
}

public nt_give_player_sec_wpn(id, iSecondaryWpnId)
{
	
	if (!native_is_valid_sw(iSecondaryWpnId))
	{
...
public native_is_valid_sw(iSecondaryWpnId)
{
...
public WU_WpnPlayAnim_Pre(id, iEnt, iWeaponId, iAnim)
{
	...
	if(iSecondaryWpnId > -1)
	{
		...
		if (iAnim == Get_CSWPN_Anim_Idle(iWeaponId, bool:cs_get_weapon_silen(iEnt)))
		{
			new iIronSightIdleAnim = Get_CSWPN_MaxAnimation(iWeaponId) + ANIM_IRONSIGHT_IDLE_3
...
public csred_WpnReload_Post(id, iEnt, iWeaponId)
{
	...
	else
	{
		...
		if (UT_Get_CS_ReloadType(iWeaponId) != 0)
		{
			...
			new Float:fWeaponReloadTime = UT_Get_CS_Reload_Time(iWeaponId)
			set_task(fWeaponReloadTime, "PrepareReloading_TASK" ,id + TASK_PREPARE_INSERT)
		}
		else
		{
...
public PrepareReloading_TASK(TASKID)
{
	...
	else
	{
		...
		iMaxClip = nt_get_sec_wpn_clip(iSecondaryWpnId)
...
public InsertAmmo_TASK(TASKID)
{
	...
	else
	{
		...
		TrieSetCell(weapon_StringInfo[iSecondaryWpnId], SECTION_INSERT_TIME, fInsertTime)
		TrieSetCell(weapon_StringInfo[iSecondaryWpnId], SECTION_AFTER_INSERT_TIME, fFinishTime)
	...
	{
		//	Finish Reloading
		tfm_finish_reloading_task(id, iEnt, iWeaponId, fFinishTime)
		return
	}
	...
	if (iClip >= iMaxClip || !iBpa || CheckPlayerBit(b_CancelReloading, id))
	{
		//	Finish Reloading
		tfm_finish_reloading_task(id, iEnt, iWeaponId, fFinishTime)
		return
...
public tfm_finish_reloading_task(id, iEnt, iWeaponId, Float:fFinishTime)
{	
	...
	remove_task(id + TASK_FINISH_INSERTING)
	set_task(fFinishTime, "FinishReloading_TASK", id + TASK_FINISH_INSERTING)
}
...
public csred_WpnDeploy_Pre(id, iEnt, iWeaponId)
{
	...
	if (!native_is_valid_sw(iSecondaryWpnId) || nt_get_sec_wpn_real_id(iSecondaryWpnId) != iWeaponId)
	{
		UT_CS_Crosshair_Toggle(id, UT_Get_CS_Crosshair(iWeaponId), 1)
...
public ToggleCrosshair_TASK(iParam[3], TASKID)
{
	...
	new iWeaponId = get_pdata_int(iEnt, m_iId, 4)//cs_get_weapon_id(iEnt)
...
{	
	...
	UT_GetWeaponExtension(iWeaponId, 0, szAnimExtension, sizeof szAnimExtension - 1)
	UT_SetPlayerSequence(id, szAnimExtension)
	
	new iDrawAnimation = Get_CSWPN_Anim_Draw(iWeaponId, 0, 0)
