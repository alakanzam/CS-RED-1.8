...
stock SPECTATOR_NVG_COLOR[] = {158, 250, 245}
...
new Float:CLASS_fHealth[MAX_CLASS]
new Float:CLASS_fGravity[MAX_CLASS]
new Float:CLASS_fKnockBack[MAX_CLASS]
new Float:CLASS_fSpeed[MAX_CLASS]
new Float:CLASS_fDamage[MAX_CLASS]
new Float:CLASS_fDmgArmor[MAX_CLASS]
new Float:CLASS_fDmgReduce[MAX_CLASS]


new NVG_COLOR[MAX_CLASS][3], NVG_ALPHA[MAX_CLASS], NVG_RADIUS[MAX_CLASS]
...
stock const AMBIENT_SOUND[][] = {"HandDraw.wav", "Hand_HitBody.wav", "Hand_HitWall.wav",
...
public nt_set_class_health(iClassId, iHealth)
{
	...
	CLASS_fHealth[iClassId] = float(iHealth)
...
public nt_set_class_gravity(iClassId, Float:fGravity)
{
	...
	CLASS_fGravity[iClassId] = fGravity
...
public nt_set_zombie_damage(iClassId, Float:fDamage)
{
	...
	CLASS_fDamage[iClassId] = fDamage
	return 1
}
...
public nt_set_zombie_armor_dmg(iClassId, Float:fArmorDamage)
{
	...
	CLASS_fDmgArmor[iClassId] = fArmorDamage
	return 1
}
...
public nt_set_zombie_dmg_reduce(iClassId, Float:fDmgReduce)
{
	...
	CLASS_fDmgReduce[iClassId] = fDmgReduce
	return 1
}
...
public nt_set_zombie_knockback(iClassId, Float:fKnockBack)
{
	...
	CLASS_fKnockBack[iClassId] = fKnockBack
...
public nt_set_zombie_NVG(iClassId, iColor[3], iAlpha, iRadius)
{
	...
	NVG_COLOR[iClassId][0] = iColor[0]
	NVG_COLOR[iClassId][1] = iColor[1]
	NVG_COLOR[iClassId][2] = iColor[2]
	
	NVG_ALPHA[iClassId] = iAlpha
	NVG_RADIUS[iClassId] = iRadius
}
...
public Float:nt_get_class_dmg_reduce(iClassId)
{
	...
	return CLASS_fDmgReduce[iClassId]
}
public Float:nt_get_zombie_gravity(iClassId)
{
	...
	return CLASS_fGravity[iClassId]
}
...
public Float:nt_get_class_kb(iClassId)
{
	...
	return CLASS_fKnockBack[iClassId]
}
...
public GamePlay_Initilizing(iCurrentGamePlay)
{
	...
	if (iCurrentGamePlay == GAMEMODE_ZM)
	{
		...
		if (file_exists(szGameModeCfg))
		{
			...
			{
				iGamePlay_Toggle = 0
				set_fail_state("[ZM] Not valid Game-Play")
				return
			}
		}
	}
	...
	{	
		...
		if (!iGamePlay_Toggle )
		{
			set_fail_state("[ZM] Map is not suitable")
			return
		}
...
public fw_PlayerTakeDamage(iVictim, iInflictor, iAttacker, Float:fDamage, iDamageBit)
{
	...
	else if (nt_get_user_zombie(iAttacker) && !nt_get_user_zombie(iVictim))// Zombie attacks Soldier
	{
		...
		fArmor -= CLASS_fDmgArmor[iClassId_Attacker]
		...
		SetHamParamFloat(4, CLASS_fDamage[iClassId_Attacker])
	...
	else
	{
		...
		new Float:fPercentage = (100.0 - CLASS_fDmgReduce[iClassId_Victim]) / 100
		new Float:fDamage = CLASS_fDamage[iClassId_Attacker] * fPercentage
		SetHamParamFloat(4, fDamage)
	}
	return HAM_IGNORED
}
...
public ZombieNightVision_TASK(TASKID)
{
	...
	//	DRAW DYNAMIC LIGHT
	draw_light(id, iOrigin, NVG_RADIUS[iClassId], NVG_COLOR[iClassId])
