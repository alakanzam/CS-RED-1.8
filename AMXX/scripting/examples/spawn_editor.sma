...
#define PLUGINNAME "CSDM Spawn Editor"
#define VERSION "1.0"
#define AUTHORS "CSDM Team"
...
public plugin_init()
{
	...
	register_dictionary("TFM_SpawnEditor.txt");
...
readSpawns()
{
	...
	if (file_exists(MapFile)) 
	{
		...
		while(g_TotalSpawns < MAX_SPAWNS && (line = read_file(MapFile , line , Data , 123 , len) ) != 0 ) 
		{
			...
			//v-Angles
			g_SpawnVAngles[g_TotalSpawns][0] = str_to_num(pos[7]);
			g_SpawnVAngles[g_TotalSpawns][1] = str_to_num(pos[8]);
			g_SpawnVAngles[g_TotalSpawns][2] = str_to_num(pos[9]);
...
//Menu Callbacks
	...
	new szKey[128];
	formatex(szKey, sizeof szKey - 1, "%L", "ITEM_ADD_TO_SPAWN");
	menu_additem(g_MainMenuID, szKey,"1", 0, g_cMain);
	
	formatex(szKey, sizeof szKey - 1, "%L", "ITEM_EDIT_CLOSEST_SPAWN");
	menu_additem(g_MainMenuID, szKey,"2", 0, g_cMain);
	
	formatex(szKey, sizeof szKey - 1, "%L", "ITEM_DELETE_CLOSEST_SPAWN");
	menu_additem(g_MainMenuID, szKey,"3", 0, g_cMain);
	
	formatex(szKey, sizeof szKey - 1, "%L", "ITEM_REFRESH_CLOSEST_SPAWN");
	menu_additem(g_MainMenuID, szKey, "4", 0, g_cMain);
	
	formatex(szKey, sizeof szKey - 1, "%L", "ITEM_SHOW_STATISTICS");
	menu_additem(g_MainMenuID, szKey, "5", 0, -1);
}
...
public c_Main(id, menu, item)
{
	...
	switch(num)
	{
		case	1:
		{
			...
			{
				formatex(fItem,sizeof fItem - 1,"%L", "SPAWN LIMIT REACHED");
				menu_item_setname(menu, item, fItem );
				return ITEM_DISABLED;
			}
			else
			{
				format(fItem,325,"%L", "ADD_POSITION_TO_SPAWN");
				menu_item_setname(menu, item, fItem );
				return ITEM_ENABLED;
			}
		}
...
add_spawn(vecs[3], angles[3], vangles[3])
{
	...
	new line[128];
	format(line, sizeof line - 1, "%d %d %d %d %d %d 0 %d %d %d",vecs[0], vecs[1], vecs[2], angles[0], angles[1], angles[2], vangles[0], vangles[1], vangles[2]);
